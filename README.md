# Messenger0
Messenger (Backend)

Link of website: https://www.messenger0.world

## License
This repository is licensed under the [Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International (CC BY-NC-ND 4.0)](https://creativecommons.org/licenses/by-nc-nd/4.0/).

### Key points of this license:
- You may view and use the code for reading or learning purposes.
- No one is allowed to modify the code or use it for commercial purposes.
- Additionally, all copyright for this project remains with the author.

For more details, see the [full license](./LICENSE).
